...
type Spider struct {
	...
	// fetcher, when set, routes crawl requests through the scanner's transport
	// pipeline (rate limit, circuit breaker, jitter, uTLS) for consistency.
	fetcher func(context.Context, *http.Request) (*http.Response, []byte, error)
}

// SetFetcher makes the spider issue requests through the given fetcher instead
// of its bare internal client.
func (s *Spider) SetFetcher(f func(context.Context, *http.Request) (*http.Response, []byte, error)) {
	s.fetcher = f
}
...
func (s *Spider) Crawl(ctx context.Context, targetURL string) (*SeedResult, error) {
	...
	// 5. Probe well-known API spec endpoints (Swagger/OpenAPI/GraphQL) and mine
	// their declared paths.
	specPaths := s.probeSpecs(ctx, targetURL)
	result.Paths = append(result.Paths, specPaths...)
	result.APIHints = append(result.APIHints, specPaths...)
...
func (s *Spider) fetch(ctx context.Context, url string) (string, error) {
	...
	// Preferred path: the scanner's transport pipeline (already reads a bounded
	// body for us).
	if s.fetcher != nil {
		resp, body, err := s.fetcher(ctx, req)
		if err != nil {
			return "", err
		}
...
// wellKnownSpecs are common locations for API documentation/spec endpoints.
var wellKnownSpecs = []string{
	"/swagger.json", "/swagger/v1/swagger.json", "/openapi.json", "/api/openapi.json",
	"/api-docs", "/v2/api-docs", "/v3/api-docs", "/api/swagger.json",
	"/swagger-ui.html", "/graphql", "/.well-known/openid-configuration",
}

var openAPIPathRegex = regexp.MustCompile(`"(/[a-zA-Z0-9_\-{}/.]{1,80})"\s*:`)

// probeSpecs requests well-known spec endpoints and, when one is present, mines
// the endpoint paths it declares so they can be fuzzed directly.
func (s *Spider) probeSpecs(ctx context.Context, targetURL string) []string {
	base := strings.TrimSuffix(targetURL, "/")
	var found []string
	for _, sp := range wellKnownSpecs {
		body, err := s.fetch(ctx, base+sp)
		if err != nil {
			continue
		}
		lb := strings.ToLower(body)
		if !(strings.Contains(lb, "swagger") || strings.Contains(lb, "openapi") ||
			strings.Contains(lb, `"paths"`) || strings.Contains(lb, "__schema") ||
			strings.Contains(lb, "graphql") || strings.Contains(lb, "authorization_endpoint")) {
			continue
		}
		found = append(found, sp)
		for i, m := range openAPIPathRegex.FindAllStringSubmatch(body, -1) {
			if i >= 200 { // bound extraction
				break
			}
			if len(m) > 1 {
