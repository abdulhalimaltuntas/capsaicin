...
func (cs *ChallengeSolver) attemptSolve(ctx context.Context, targetURL string) (*SolvedSession, error) {
	...
	// Cache the session keyed by host so GetCachedSession(host) can find it.
	cs.mu.Lock()
	cs.cache[hostFromURL(targetURL)] = session
	cs.mu.Unlock()

	return session, nil
}

// hostFromURL returns the host[:port] of a URL, falling back to the raw string.
