...
func TestValidateConfig_ResilienceFlags(t *testing.T) {
	wordlist, err := os.CreateTemp("", "wordlist-*.txt")
	if err != nil {
		t.Fatal(err)
	}
	...
	// Zero cb-threshold/cb-reset are valid (mean "use default").
	if err := ValidateConfig(base()); err != nil {
		t.Fatalf("zero resilience flags should be valid: %v", err)
	}

	// Negative max-duration is rejected.
	neg := base()
	neg.MaxDuration = -5
	if err := ValidateConfig(neg); err == nil {
		t.Error("expected error for negative --max-duration")
	}

	// Negative cb-threshold is rejected.
	cb := base()
	cb.CBThreshold = -1
	if err := ValidateConfig(cb); err == nil {
		t.Error("expected error for negative --cb-threshold")
	}
	...
	ok.MaxDuration = 900
	ok.CBThreshold = 30
	ok.CBResetSeconds = 45
	if err := ValidateConfig(ok); err != nil {
		t.Errorf("explicit resilience values should validate: %v", err)
	}
}
